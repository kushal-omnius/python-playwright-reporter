...
exports.generateTrendChart = generateTrendChart;
...
function generateTrendChart(data) {
    ...
    const generateBarChart = (chartData, getValue, maxValue, color, yAxisLabel, formatValue, avgColor = '#ffcc00') => {
      ...
      `;
        ...
        const bars = chartData.map((d, i) => {
            ...
            const label = i === chartData.length - 1 ? 'Current' : (0, utils_1.formatShortDate)(d.timestamp);
            const isCurrent = i === chartData.length - 1;
            const displayValue = formatValue ? formatValue(value) : value.toString();
            const runId = d.runId || '';
            const safeRunId = (0, sanitizers_1.escapeHtml)(runId).replace(/'/g, "\\'");
            const clickable = !isCurrent && runId;
            const isAnomaly = anomalies[i];
            return `
        <g class="bar-group${clickable ? ' clickable' : ''}" data-tooltip="${label}: ${displayValue}${isAnomaly ? ' ⚠ Anomaly' : ''}" ${clickable ? `data-runid="${(0, sanitizers_1.escapeHtml)(runId)}" onclick="loadHistoricalRun('${safeRunId}', '${label}')"` : ''}>
          <rect
            x="${x}"
            y="${y}"
      ...
      `;
        ...
        const xLabels = chartData.map((d, i) => {
            if (chartData.length > 10 && i % 2 !== 0 && i !== chartData.length - 1)
                return '';
            const x = (padding.left + i * spacing + spacing / 2).toFixed(1);
            const label = i === chartData.length - 1 ? 'Now' : (0, utils_1.formatShortDate)(d.timestamp);
    ...
    const passRateChart = generateBarChart(allSummaries, (s) => s.passRate || 0, 100, '#22c55e', 'Pass Rate (%)', (val) => `${val}%`);
    // Generate duration bar chart
    const durationChart = generateBarChart(allSummaries, (s) => Math.round((s.duration || 0) / 1000), Math.ceil(maxDuration / 1000), '#a855f7', 'Duration (s)', (val) => `${val}s`);
    // Generate flaky tests bar chart
    const flakyChart = generateBarChart(allSummaries, (s) => s.flaky || 0, maxFlaky, '#eab308', 'Flaky Tests');
    // Generate slow tests bar chart
    const slowChart = generateBarChart(allSummaries, (s) => s.slow || 0, maxSlow, '#f97316', 'Slow Tests');
    return `
