 ...
 */
function generateTestCard(test, showTraceSection) {
    ...
    const tagsAttr = test.tags && test.tags.length > 0 ? ` data-tags="${test.tags.map(t => (0, utils_1.escapeHtml)(t)).join(',')}"` : '';
    const suiteAttr = test.suite ? ` data-suite="${(0, utils_1.escapeHtml)(test.suite)}"` : '';
    const suitesAttr = test.suites && test.suites.length > 0 ? ` data-suites="${test.suites.map(s => (0, utils_1.escapeHtml)(s)).join(',')}"` : '';
    // Browser/project data attributes for filtering
    const browserAttr = test.browser ? ` data-browser="${(0, utils_1.escapeHtml)(test.browser)}"` : '';
    ...
    const suiteHtml = test.suite
        ? `<span class="test-suite-badge" title="Suite: ${test.suites?.map(s => (0, utils_1.escapeHtml)(s)).join(' > ') || (0, utils_1.escapeHtml)(test.suite)}">${(0, utils_1.escapeHtml)(test.suite)}</span>`
        : '';
    // Generate browser badge display (for multi-browser setups)
    const browserHtml = test.browser
        ? `<span class="test-browser-badge" title="Browser: ${(0, utils_1.escapeHtml)(test.browser)}">${getBrowserIcon(test.browser)} ${(0, utils_1.escapeHtml)(test.browser)}</span>`
        : '';
    // Generate project badge display (for multi-project setups)
    const projectHtml = test.project && test.project !== test.browser
        ? `<span class="test-project-badge" title="Project: ${(0, utils_1.escapeHtml)(test.project)}">${(0, utils_1.escapeHtml)(test.project)}</span>`
        : '';
    ...
    const annotationsHtml = test.annotations && test.annotations.length > 0
        ? test.annotations.map(a => {
            ...
            const title = a.description ? `${a.type}: ${(0, utils_1.escapeHtml)(a.description)}` : a.type;
            ...
            return `<span class="test-annotation-badge annotation-${cssType}" title="${(0, utils_1.escapeHtml)(title)}">${icon} ${(0, utils_1.escapeHtml)(a.type)}</span>`;
        }).join('')
        : '';
        ...
        <div class="test-card-left">
           <div class="test-info">
            ...
            <div class="test-meta-row">
              <span class="test-file">${(0, utils_1.escapeHtml)(test.file)}</span>
            </div>${badgesHtml}
          </div>
        </div>
 ...
 */
function generateTestDetails(test, cardId, showTraceSection) {
    let historyDetails = '';
    let bodyDetails = '';
    ...
    if (test.history && test.history.length > 0) {
        ...
        historyDetails += `
	      <div class="detail-section">
            ...
            const timestampLabel = (0, utils_1.escapeHtml)(formatHistoryTimestamp(h.timestamp));
            const statusLabel = h.skipped ? 'Skipped' : h.passed ? 'Passed' : 'Failed';
            const runIdAttr = h.runId ? ` data-runid="${(0, utils_1.escapeHtml)(h.runId)}"` : '';
            return `<div class="spark-dot history-dot ${h.skipped ? 'skip' : h.passed ? 'pass' : 'fail'}"${runIdAttr} data-testid="${(0, utils_1.escapeHtml)(test.testId)}" data-ts="${timestampLabel}" title="Run ${i + 1}: ${statusLabel} • ${timestampLabel}"></div>`;
        }).join('')}
            ...
            const runIdAttr = h.runId ? ` data-runid="${(0, utils_1.escapeHtml)(h.runId)}"` : '';
            return `<div class="duration-bar history-duration"${runIdAttr} style="height: ${height}px" title="Run ${i + 1}: ${(0, utils_1.formatDuration)(h.duration)}"></div>`;
        }).join('')}
              <div class="duration-bar current ${currentTrendClass}" style="height: ${maxDuration > 0 ? Math.max(4, (test.duration / maxDuration) * 28).toFixed(1) : '4'}px" title="Current: ${(0, utils_1.formatDuration)(test.duration)}"></div>
            </div>
            <div class="history-stats">
              <span class="history-stat">Avg: <span data-role="avg-duration">${(0, utils_1.formatDuration)(avgDuration)}</span></span>
              <span class="history-stat">Current: <span data-role="current-duration">${(0, utils_1.formatDuration)(test.duration)}</span></span>
            </div>
          </div>
        </div>
      </div>
    ...
    if (test.steps.length > 0) {
        ...
        function categorizeStep(title) {
            ...
            if (t.includes('goto') || t.includes('navigate') || t.includes('page.goto'))
                return 'navigation';
            if (t.includes('expect') || t.includes('assert') || t.includes('toHave') || t.includes('toBe'))
                return 'assertion';
            if (t.includes('click') || t.includes('fill') || t.includes('type') || t.includes('press') || t.includes('check') || t.includes('select'))
                return 'action';
            if (t.includes('wait') || t.includes('timeout'))
                return 'wait';
        ...
        const timelineBars = test.steps.map(step => {
            ...
            const label = widthPct > 8 ? (0, utils_1.escapeHtml)(step.title.length > 20 ? step.title.slice(0, 18) + '..' : step.title) : '';
            return `<div class="step-timeline-bar cat-${cat}" style="left: ${leftPct.toFixed(1)}%; width: ${Math.max(widthPct, 0.5).toFixed(1)}%;" title="${(0, utils_1.escapeHtml)(step.title)} (${(0, utils_1.formatDuration)(step.duration)})">${label}</div>`;
        }).join('');
        const catColors = { navigation: '#3b82f6', assertion: '#22c55e', action: '#a855f7', api: '#f59e0b', wait: '#6b7280', other: '#64748b' };
        const catLabels = { navigation: 'Navigation', assertion: 'Assertion', action: 'Action', api: 'API', wait: 'Wait', other: 'Other' };
        const legendHtml = Array.from(usedCategories).map(cat => `<span class="step-timeline-legend-item"><span class="step-timeline-legend-dot" style="background: ${catColors[cat]}"></span>${catLabels[cat]}</span>`).join('');
        bodyDetails += `
      <div class="detail-section">
    ...
    `;
        ...
        <div class="diff-container">
          ...
          <div class="diff-line diff-expected">${(0, utils_1.escapeHtml)(expected)}</div>
          <div class="diff-line diff-actual">${(0, utils_1.escapeHtml)(received)}</div>
        </div>
      `;
        }
        bodyDetails += `
      ...
      <div class="detail-section">
        ...
        <div class="trace-list">
          ${tracePaths.map((trace, idx) => {
            ...
            const safeTrace = (0, utils_1.escapeHtml)(trace);
            const fileName = (0, utils_1.escapeHtml)(trace.split(/[\\\\/]/).pop() || trace);
            return `
              <div class="trace-row">
    ...
    if (test.attachments?.custom && test.attachments.custom.length > 0) {
        const customAttachmentsList = test.attachments.custom.map(att => {
            ...
            if (att.path) {
                return `<a href="file://${(0, utils_1.escapeHtml)(att.path)}" class="attachment-link" target="_blank">${icon} ${(0, utils_1.escapeHtml)(att.name)}</a>`;
            }
            else if (att.body) {
                // Inline content - create a download link
                const dataUri = `data:${att.contentType};base64,${att.body}`;
                return `<a href="${dataUri}" class="attachment-link" download="${(0, utils_1.escapeHtml)(att.name)}">${icon} ${(0, utils_1.escapeHtml)(att.name)}</a>`;
            }
            return `<span class="attachment-name">${icon} ${(0, utils_1.escapeHtml)(att.name)}</span>`;
        }).join('');
        bodyDetails += `
      <div class="detail-section">
    ...
    if (test.averageDuration !== undefined) {
      ...
      <div class="duration-compare">
        Average: ${(0, utils_1.formatDuration)(test.averageDuration)} → Current: ${(0, utils_1.formatDuration)(test.duration)}
      </div>
    `;
    }
 ...
 */
function generateGroupedTests(results, showTraceSection, attention = { newFailures: new Set(), regressions: new Set(), fixed: new Set() }) {
    // Group tests by file
    ...
    return Array.from(groups.entries()).map(([file, tests]) => {
        ...
        <div class="test-list-item ${statusClass}" 
             id="list-item-${cardId}"
             data-testid="${(0, utils_1.escapeHtml)(test.testId)}"
             data-status="${test.status}"
             data-flaky="${isFlaky}"
             data-slow="${isSlow}"
             data-new="${isNew}"
          ...
          <div class="test-item-info">
            <div class="test-item-title">${(0, utils_1.escapeHtml)(test.title)}</div>
          </div>
          <div class="test-item-meta">
            ...
            ${stabilityBadge}
            <span class="test-item-duration">${(0, utils_1.formatDuration)(test.duration)}</span>
 ...
 */
function generateNetworkLogsSection(networkLogs, cardId) {
    ...
    const entriesHtml = entries.map((entry, idx) => {
        ...
        <div class="network-body request-body">
          ...
          <pre class="network-body-content">${(0, utils_1.escapeHtml)(truncated)}</pre>
        </div>
      `;
        }
        return `
      <div class="network-entry ${statusClass}" data-entry-id="${entryId}">
        <div class="network-entry-header" onclick="toggleNetworkEntry('${entryId}')">
          ...
          <span class="network-url" title="${(0, utils_1.escapeHtml)(entry.url)}">${(0, utils_1.escapeHtml)(displayUrl)}</span>
        ...
        <div class="network-entry-details" id="${entryId}-details" style="display: none;">
          ...
          <div class="network-meta">
            <div class="network-meta-item">
              ...
              <span class="meta-value">${(0, utils_1.escapeHtml)(entry.contentType || 'unknown')}</span>
            </div>
            <div class="network-meta-item">
