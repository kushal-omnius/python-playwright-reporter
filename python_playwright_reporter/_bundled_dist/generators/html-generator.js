...
const trace_viewer_generator_1 = require("./trace-viewer-generator");
/**
 * Generate file tree structure for sidebar
 */
function generateFileTree(results) {
    ...
    return Array.from(fileGroups.entries()).map(([file, stats]) => {
        ...
        return `
      <div class="file-tree-item ${statusClass}" data-file="${(0, utils_1.escapeHtml)(file)}" onclick="filterByFile('${(0, utils_1.escapeHtml)(file)}')">
        <span class="file-tree-icon">📄</span>
        <span class="file-tree-name" title="${(0, utils_1.escapeHtml)(file)}">${(0, utils_1.escapeHtml)(fileName)}</span>
 ...
 */
function generateTestListItems(results, showTraceSection, attention = { newFailures: new Set(), regressions: new Set(), fixed: new Set() }) {
    return results.map(test => {
      ...
      <div class="test-list-item ${statusClass}"
           id="list-item-${cardId}"
           role="listitem"
           aria-label="${(0, utils_1.escapeHtml)(test.title)} - ${statusLabel}"
        ...
        <div class="test-item-info">
          ...
          <div class="test-item-file">${(0, utils_1.escapeHtml)(test.file)}</div>
        </div>
        ...
        return `
          <div class="cluster-card" onclick="filterTests('failed'); switchView('tests');">
            <div class="cluster-header">
              ...
              <div class="cluster-type">${(0, utils_1.escapeHtml)(cluster.errorType)}</div>
              <div class="cluster-count">${cluster.count} test${cluster.count > 1 ? 's' : ''}</div>
            </div>
            ${errorPreview ? `<div class="cluster-error">${(0, utils_1.escapeHtml)(errorPreview)}</div>` : ''}
            <div class="cluster-tests">
              ${cluster.tests.slice(0, 3).map(t => `<span class="cluster-test-name">${(0, utils_1.escapeHtml)(t.title)}</span>`).join('')}
              ${cluster.tests.length > 3 ? `<span class="cluster-more">+${cluster.tests.length - 3} more</span>` : ''}
            </div>
            <div class="cluster-files">
              ${affectedFiles.slice(0, 2).map(f => `<span class="cluster-file">${(0, utils_1.escapeHtml)(f)}</span>`).join('')}
  ...
  ` : '';
    ...
    <div class="overview-section">
      ...
      <div class="insights-grid">
        ${slowestTest ? `
          <div class="insight-card" onclick="selectTest('${(0, utils_1.sanitizeId)(slowestTest.testId)}'); switchView('tests');">
            <div class="insight-icon">🐢</div>
            <div class="insight-content">
              <div class="insight-label">Slowest Test</div>
              <div class="insight-title">${(0, utils_1.escapeHtml)(slowestTest.title)}</div>
              <div class="insight-value">${(0, utils_1.formatDuration)(slowestTest.duration)}</div>
            </div>
          </div>
        ` : ''}
        ${mostFlakyTest && mostFlakyTest.flakinessScore && mostFlakyTest.flakinessScore > 0 ? `
          <div class="insight-card" onclick="selectTest('${(0, utils_1.sanitizeId)(mostFlakyTest.testId)}'); switchView('tests');">
            <div class="insight-icon">⚡</div>
            <div class="insight-content">
              <div class="insight-label">Most Flaky Test</div>
              <div class="insight-title">${(0, utils_1.escapeHtml)(mostFlakyTest.title)}</div>
...
function generateHtml(data) {
    ...
    const allSuites = new Map(); // suite -> count
    ...
    const lightenedHistorySnapshots = enableHistoryDrilldown && historyRunSnapshots
        ? Object.fromEntries(Object.entries(historyRunSnapshots).map(([runId, snapshot]) => [
            runId,
            {
                ...snapshot,
                tests: Object.fromEntries(Object.entries(snapshot.tests || {}).map(([testId, testSnap]) => [
                    testId,
                    {
                        ...testSnap,
...
<body>
  ...
  <div class="app-shell" role="application">
    ...
    <div class="ci-info-bar">
      <span class="ci-provider">${(0, utils_1.escapeHtml)(ciInfo.provider.toUpperCase())}</span>
      ${ciInfo.branch ? `<span class="ci-item"><span class="ci-label">Branch:</span> ${(0, utils_1.escapeHtml)(ciInfo.branch)}</span>` : ''}
      ${ciInfo.commit ? `<span class="ci-item"><span class="ci-label">Commit:</span> <code>${(0, utils_1.escapeHtml)(ciInfo.commit)}</code></span>` : ''}
      ${ciInfo.buildId ? `<span class="ci-item"><span class="ci-label">Build:</span> #${(0, utils_1.escapeHtml)(ciInfo.buildId)}</span>` : ''}
    </div>
    ` : ''}
      ...
      <div class="sidebar-filters" role="region" aria-label="Test filters">
        ...
        <div class="filter-group" data-group="suite" role="group" aria-label="Suite filters">
          ...
          <div class="filter-chips suite-chips" role="group" aria-labelledby="suite-filter-label">
            ${sortedSuites.slice(0, 8).map(([suite, count]) => `<button class="filter-chip suite-chip" data-filter="suite-${(0, utils_1.escapeHtml)(suite)}" data-group="suite" data-suite-name="${(0, utils_1.escapeHtml)(suite)}" onclick="toggleFilter(this)" aria-pressed="false" title="${(0, utils_1.escapeHtml)(suite)} (${count} tests)">${(0, utils_1.escapeHtml)(suite.length > 15 ? suite.slice(0, 12) + '...' : suite)} (${count})</button>`).join('')}
          </div>
        </div>
        ` : ''}
        ${sortedTags.length > 0 ? `
        <div class="filter-group" data-group="tag" role="group" aria-label="Tag filters">
          ...
          <div class="filter-chips tag-chips" role="group" aria-labelledby="tag-filter-label">
            ${sortedTags.slice(0, 8).map(([tag, count]) => `<button class="filter-chip tag-chip" data-filter="tag-${(0, utils_1.escapeHtml)(tag)}" data-group="tag" data-tag-name="${(0, utils_1.escapeHtml)(tag)}" onclick="toggleFilter(this)" aria-pressed="false" title="${(0, utils_1.escapeHtml)(tag)} (${count} tests)">${(0, utils_1.escapeHtml)(tag)} (${count})</button>`).join('')}
          </div>
        </div>
        ` : ''}
      </div>
    ...
    <main class="main-content" id="main-content" tabindex="-1" aria-label="Test report content">
          ...
          <div class="test-list-panel">
            ...
            <div class="test-list-content">
              ...
              <div class="test-tab-content" id="tab-by-file" role="tabpanel" aria-labelledby="tab-by-file-label">
                ${(0, card_generator_1.generateGroupedTests)(sortedResults, showTraceSection, attentionSets)}
              </div>
              <!-- By Status Tab -->
      ...
      <section class="view-panel" id="view-trends" role="tabpanel" aria-label="Trends" style="display: none;">
        ...
        <div class="trends-content">
          ${(0, chart_generator_1.generateTrendChart)({ results, history, startTime })}
        </div>
      </section>
      ...
      <section class="view-panel" id="view-comparison" role="tabpanel" aria-label="Comparison" style="display: none;">
        ...
        <div class="comparison-content">
          ${(0, comparison_generator_1.generateComparison)(comparison)}
        </div>
      </section>
      ` : ''}
      ...
      <section class="view-panel" id="view-gallery" role="tabpanel" aria-label="Gallery" style="display: none;">
        ...
        <div class="gallery-content">
          ${(0, gallery_generator_1.generateGallery)(results)}
        </div>
      </section>
      ` : ''}
    </main>
  ...
  ${enableTraceViewer ? (0, trace_viewer_generator_1.generateTraceViewerHtml)() : ''}

  <!-- Hidden data containers for detail rendering -->
  <div id="test-cards-data" style="display: none;">
    ${results.map(test => (0, card_generator_1.generateTestCard)(test, showTraceSection)).join('\n')}
  </div>

  <!-- Issue #13: JSZip library for trace extraction -->
  ${enableTraceViewer ? `<script>${(0, trace_viewer_generator_1.generateJSZipScript)()}</script>` : ''}
...
${enableTraceViewer ? (0, trace_viewer_generator_1.generateTraceViewerScript)() : ''}
  </script>
</body>
</html>`;
}
 ...
 */
function generateStyles(passRate, cspSafe = false) {
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    ${(0, trace_viewer_generator_1.generateTraceViewerStyles)(monoFont)}
`;
}
/**
 * Generate all JavaScript for the new app-shell layout
 */
function generateScripts(testsJson, includeGallery, includeComparison, enableTraceViewer, enableHistoryDrilldown, historyRunSnapshotsJson, statsData) {
...
${includeGallery ? `    // Gallery functions\n${(0, gallery_generator_1.generateGalleryScript)()}` : ''}

${includeComparison ? `    // Comparison functions\n${(0, comparison_generator_1.generateComparisonScript)()}` : ''}
